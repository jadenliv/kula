...
import { TagPills } from '../components/notes/NotesPanel'
...
export default function Notebook() {
  ...
  const filteredNotes = useMemo(
    () => allNotes.filter((n) => matchesSearch(n, debouncedQuery) && matchesTags(n, activeTags)),
    [allNotes, debouncedQuery, activeTags],
  )

  const groups = useMemo(() => groupNotesBySefer(filteredNotes), [filteredNotes])
  // Export always uses all notes, not just filtered view
  ...
  const isFiltered = debouncedQuery.trim() !== '' || activeTags.length > 0
      ...
      {!isLoading && !error && allNotes.length === 0 && (
        <div className="rounded-xl border border-dashed border-[var(--border)] bg-[var(--surface-raised)] px-6 py-12 text-center">
          <p className="text-sm text-kula-500 dark:text-kula-400">
            ...
            <Link to="/browse" className="font-medium text-kula-600 underline-offset-2 hover:underline dark:text-kula-300">
              Browse
            </Link>{' '}
  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between gap-3">
        ...
        <time dateTime={note.created_at} className="shrink-0 text-xs text-kula-400 dark:text-kula-600">
