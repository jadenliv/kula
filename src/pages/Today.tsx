...
function formatGregorianDate(short = false): string {
  if (short) {
...
export default function Today() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 md:space-y-8">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-sm text-kula-600 dark:text-kula-400 md:hidden">
            ...
            {hebrewDate && (
              <span className="ml-2 font-['Heebo'] text-kula-500 dark:text-kula-400">
                · {hebrewDate}
              </span>
            )}
          </p>
          <p className="mt-1 hidden flex-wrap items-baseline gap-x-3 text-sm text-kula-600 dark:text-kula-400 md:flex">
            <span>{formatGregorianDate()}</span>
            {hebrewDate && (
              <>
                <span className="text-kula-300 dark:text-kula-700">·</span>
                <span className="font-['Heebo'] text-kula-700 dark:text-kula-300">{hebrewDate}</span>
              </>
            )}
          </p>
        </div>
        ...
        <button
          ...
          className="flex shrink-0 items-center gap-1.5 rounded-lg border border-[var(--border)] px-2.5 py-1.5 text-xs text-kula-600 transition-colors hover:border-kula-400 hover:text-kula-500 dark:text-kula-400 dark:hover:text-kula-300"
        >
          <svg viewBox="0 0 20 20" className="h-3.5 w-3.5" fill="currentColor">
            <path fillRule="evenodd" d="M9.69 18.933l.003.001C9.89 19.02 10 19 10 19s.11.02.308-.066l.002-.001.006-.003.018-.008a5.741 5.741 0 00.281-.14c.186-.096.446-.24.757-.433.62-.384 1.445-.966 2.274-1.765C15.302 14.988 17 12.493 17 9A7 7 0 103 9c0 3.492 1.698 5.988 3.355 7.584a13.731 13.731 0 002.273 1.765 11.842 11.842 0 00.976.544l.062.029.018.008.006.003zM10 11.25a2.25 2.25 0 100-4.5 2.25 2.25 0 000 4.5z" clipRule="evenodd" />
          </svg>
          <span className="hidden sm:inline">{location ? location.label : 'Set location'}</span>
          <span className="sm:hidden">{location ? '✓' : 'Location'}</span>
        </button>
      </div>
      ...
      {/* Zmanim section */}
      <section>
        <h3 className="mb-3 font-serif text-lg tracking-tight text-kula-900 dark:text-kula-100 md:mb-4 md:text-xl">
          זמנים <span className="ml-2 font-sans text-xs font-normal text-kula-500 md:text-sm">Zemanim</span>
        </h3>
      ...
      <section>
        <h3 className="mb-3 font-serif text-lg tracking-tight text-kula-900 dark:text-kula-100 md:mb-4 md:text-xl">
          לימוד היומי <span className="ml-2 font-sans text-xs font-normal text-kula-500 md:text-sm">Daily Learning</span>
        </h3>
